...
function new_show_price(url,act,func,elem_id,ifhuanxin){
	...
	if(act	==	'show_price'){		//查看价格
        ...
        var idArray = property_ids+','+desc_ids;
        $.post("imgAndPrice/price/"+modelId+"/"+idArray+"",{},
			function(data){
        		alert(data);
        		localStorage.setItem("sum",data[0]);
        		localStorage.setItem("img",data[1]);
